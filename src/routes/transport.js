...
transportRouter.get("/:type/search", async (req, res) => {
  ...
  if (!routes.length && req.query.from && req.query.to) {
    ...
    const options = type === "bus"
      ...
      : [
        ["Orbita Travels Select", 8, 16, 1, "seater"],
        ["Orbita Travels Comfort", 10, 19, 1.22, "seater"],
        ["Orbita Travels Luxe", 13, 21, 1.48, "seater"]
      ];
...
transportRouter.get("/:type/:id/points", async (req, res) => {
  ...
  res.json({
    boardingPoints: [`${route.origin} Central`, `${route.origin} Circle`, `${route.origin} Bus Terminal`].map((name, index) => ({ id: index + 1, name })),
    droppingPoints: [`${route.destination} Central`, `${route.destination} Market`, `${route.destination} Bus Terminal`].map((name, index) => ({ id: index + 1, name }))
  });
});
